...
from catalog import load_products, get_categories, get_products_by_category, get_product_by_id, format_price
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        "🚚 Доставка по России\n"
        "⭐ Оплата Telegram Stars доступна для цифровых позиций\n\n"
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "catalog":
        ...
        await query.answer("Эта функция будет активна после настройки Stars-направления.", show_alert=True)


async def show_categories(query):
    cats = get_categories(PRODUCTS)
    keyboard = []
    for c in cats:
        keyboard.append([InlineKeyboardButton(c, callback_data=f"cat_{c}")])
...
async def show_products_by_category(query, category):
    ...
    for p in products:
        ...
        keyboard.append([InlineKeyboardButton(f"{status} {p['brand']} — {format_price(p['price'])} ₽", callback_data=f"prod_{p['id']}")])
...
async def show_how_to_order(query):
    text = (
        ...
        "💳 Оплата: карта, СБП\n"
        "⭐ Для цифровых товаров можно использовать Telegram Stars\n"
...
async def show_stars_info(query):
    text = (
        "⭐ Telegram Stars\n\n"
        "Звёзды Telegram можно использовать для цифровых товаров и сервисов внутри Telegram.\n"
        "Для физических товаров обычно используют обычную оплату, а Stars — для digital/offers, бонусов, доступа и платных материалов.\n\n"
        "В этом проекте Stars подключены как отдельная витрина оплаты."
    )
    keyboard = [
        [InlineKeyboardButton("⭐ Товары за Stars", callback_data="catalog")],
        [InlineKeyboardButton("🏠 Главное меню", callback_data="back_main")]
    ]
    await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard))
...
async def show_bestsellers(query):
    ...
    keyboard = []
    for p in products:
        keyboard.append([InlineKeyboardButton(f"⭐ {p['brand']} — {format_price(p['price'])} ₽", callback_data=f"prod_{p['id']}")])
    keyboard.append([InlineKeyboardButton("🏠 Главное меню", callback_data="back_main")])
    await query.edit_message_text("🔥 Хиты продаж:", reply_markup=InlineKeyboardMarkup(keyboard))
...
def build_app():
    ...
    app.add_handler(MessageHandler(filters.PRECHECKOUT_QUERY, precheckout_handler))
    return app
